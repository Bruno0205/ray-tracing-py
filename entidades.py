...
class Esfera: #Representa uma esfera 3D
    def get_bounds(self):
        # Retorna (min_point, max_point) como Ponto
        min_point = Ponto(
...
class Mesh:
    def get_bounds(self):
        ...
        min_point = Ponto(min(xs), min(ys), min(zs))
        max_point = Ponto(max(xs), max(ys), max(zs))
        return min_point, max_point #representa uma malha
